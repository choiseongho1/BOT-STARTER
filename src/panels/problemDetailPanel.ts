...
type PanelMessage =
  | { type: "openWeb"; problemId: number }
  | { type: "copyInput"; value: string }
  | { type: "copyOutput"; value: string }
  | { type: "runAll" }
  | { type: "runCase"; caseKey: string }
  | { type: "addCase"; input: string; output: string }
  | { type: "updateCase"; caseId: string; input: string; output: string }
  | { type: "deleteCase"; caseId: string };

interface PanelTestCase {
  key: string;
  index: number;
  input: string;
  output: string;
...
export class ProblemDetailPanel {
  ...
  private currentDetail?: ProblemDetail;
  private currentCases: PanelTestCase[] = [];

  constructor(
    private readonly detailService: ProblemDetailService,
  ...
  private async handleUpdateCase(caseId: string, input: string, output: string): Promise<void> {
    if (!this.currentDetail) {
      await this.postRunStatus("문제 정보를 먼저 로드해 주세요.", true);
      return;
    }
  ...
  private async handleDeleteCase(caseId: string): Promise<void> {
    if (!this.currentDetail) {
      await this.postRunStatus("문제 정보를 먼저 로드해 주세요.", true);
      return;
    }
  ...
  private async handleRun(message: Extract<PanelMessage, { type: "runAll" | "runCase" }>): Promise<void> {
    if (!this.currentDetail) {
      await this.postRunStatus("문제 정보를 먼저 로드해 주세요.", true);
      return;
    }
    ...
    await this.postRunStatus(
      message.type === "runAll"
        ? "모든 테스트케이스 실행 중..."
        : `케이스 ${runCases[0].index} 실행 중...`,
      false
    );
  ...
  private async refreshUserCases(statusText?: string): Promise<void> {
    if (!this.currentDetail) {
      return;
    }
  ...
  private toRunnerCases(cases: PanelTestCase[]): ProblemTestCase[] {
  ...
  private buildCases(detail: ProblemDetail, userCases: ReturnType<UserTestCaseService["list"]>): PanelTestCase[] {
    const sampleCases: PanelTestCase[] = detail.testCases.map((testCase) => ({
    ...
    const customCases: PanelTestCase[] = userCases.map((testCase, offset) => ({
  ...
  private async postRunStatus(text: string, isError: boolean): Promise<void> {
    await this.panel?.webview.postMessage({
      type: "runStatus",
      text,
      isError
  ...
  private parseMessage(rawMessage: unknown): PanelMessage | undefined {
  ...
  private getHtml(detail: ProblemDetail, cases: PanelTestCase[]): string {
...
<body>
  ...
  private renderStatementSection(html: string, text: string, fallback: string): string {
  ...
  private renderTestCasesHtml(cases: PanelTestCase[]): string {
    if (cases.length === 0) {
    ...
    return cases
      .map((testCase) => {
        ...
        const userActions =
          testCase.source === "user" && testCase.caseId
